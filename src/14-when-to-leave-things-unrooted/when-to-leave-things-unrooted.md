...
In this tutorial we've mostly used rooting targets to ensure the managed data we created would remain valid as long as we didn't leave the scope it was tied to. In many cases, though, we don't need to root managed data and can use a weak target without running into any problems.
...
fn main() {
    ...
    handle.local_scope::<_, 1>(|mut frame| {
        ...
        let res = unsafe { func.call(&frame, [a]).expect("caught exception") };
