...
Unrooted data is used to represent non-inlined fields to account for mutability, which can render the field's old value unreachable. Managed types and unrooted data make use of the `Option<NonNull>` niche optimization to guarantee `Option<ValueRef>` has the same size as a pointer.[^1] We'll say that instances of `Outer` reference managed data.
